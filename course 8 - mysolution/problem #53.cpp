...
int  DaysUntilTheEndOfYear(sDate Date)
{
	...
	short TotalDays = IsLeapYear(Date.Year)? 366 :365;

	int DayFromTheBeginigOfYear = NumberOfDaysFromTheBeginingOfTheYear(Date.Year, Date.Month, Date.Day);

	return TotalDays - DayFromTheBeginigOfYear;
	

	//Another way
	/*
	sDate EndOfTheYearDate;
	EndOfTheYearDate.Day = 31;
	EndOfTheYearDate.Month = 12;
	EndOfTheYearDate.Year = Date.Year;

	return GetDifferenceInDays(Date, EndOfTheYearDate, true);
	*/
}
